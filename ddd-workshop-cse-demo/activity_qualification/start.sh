...
trap 'ps -ax | grep -w activity_qualification |grep -v grep |cut -c 1-6 |xargs kill -15;sleep 5s '  SIGTERM
#nohup java $CMDVAR -jar activity_qualification-1.0.0-SNAPSHOT.jar 2>&1 &
nohup java -jar activity_qualification-1.0.0-SNAPSHOT.jar 2>&1 &
wait $!
